...
const ServiceBill = () => {
  ...
  const [serviceBillNumber] = useState(
  ...
  // First initialize parts so setParts exists
  const parts = useParts(emptyPart, incoming);

  // Then pass setParts into serviceLookup
  const serviceLookup = useServiceLookup(
    setCustomerInfo,
    parts.setParts,
    setLaborCost,
    setPaymentMethod,
  ...
  const { printInvoice, exportPdf } = useInvoiceExport(
    invoiceRef,
    serviceBillNumber
  );
  ...
  useEffect(() => {
    if (incoming) {
      ...
      const partsFromRequest = Array.isArray(
        incoming?.serviceDetails?.serviceParts
      )
  ...
  const handleSaveServiceBill = async () => {
    // TODO: Validation and save logic here
  };
