...
const AccessoriesStock = () => {
  ...
  const formatCodeRange = (codes) => {
    ...
    // Group by prefix; pick the largest group to summarize (common case: one prefix)
    const groups = parsed.reduce((acc, p) => {
      const k = p.prefix + '|' + p.width
      if (!acc[k]) acc[k] = { prefix: p.prefix, width: p.width, nums: [] }
      acc[k].nums.push(p.num)
      return acc
    }, {})
    const best = Object.values(groups).sort((a,b)=>b.nums.length - a.nums.length)[0]
    const min = Math.min(...best.nums)
    const max = Math.max(...best.nums)
    const pad = (n) => String(n).padStart(best.width, '0')
    if (min === max) return `${best.prefix}${pad(min)}`
    return `${best.prefix}${pad(min)} to ${best.prefix}${pad(max)}`
  }
              ...
              <tbody>
                {filtered.map((r)=> (
                  <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-2 pr-4 max-w-xs whitespace-pre-wrap break-words">{formatCodeRange(r.productIds)}</td>
